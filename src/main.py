...
from experiment.module import set_module, create_atom
...
async def gather_results(
    config: DatasetConfig, dataset: str, testset: List[Dict[str, Any]]
) -> List[Any]:
    ...
    if config.requires_context():
        ...
        if isinstance(question_key, list):
            formatted_questions = [
                format_question_from_keys(item, question_key) for item in testset
            ]
            # tasks from atom
            tasks = [
                create_atom(question, contexts(item, dataset))
                for question, item in zip(formatted_questions, testset)
            ]
        else:
            tasks = [
                create_atom(item[question_key], contexts(item, dataset))
                for item in testset
            ]
    else:
        # Handle case where question_key is a list
        if isinstance(question_key, list):
            tasks = [
                create_atom(format_question_from_keys(item, question_key))
                for item in testset
            ]
        else:
            tasks = [create_atom(item[question_key]) for item in testset]
...
def construct_entry(
    ...
    scoring_function = getattr(
        __import__(f"experiment.utils", fromlist=[config.scoring_function]),
...
def update_score_log(
    start: int, end: int, dataset: str, interval: str, accuracy: float
) -> None:
    # Update score log
    log_entry = {
        "start": start,
...
async def main():
    ...
    parser.add_argument(
        "--start", type=int, default=0, help="Start index of the dataset"
    )
    parser.add_argument(
        "--end", type=int, default=2, help="End index of the dataset (-1 for all)"
    )
    parser.add_argument(
        "--model",
        type=str,
